...
export function CheckoutModal({ open, onOpenChange }: CheckoutModalProps) {
  ...
  useEffect(() => {
    if (session?.user && open) {
      // First auto-fill basic user info immediately
      setFormData(prev => ({
        ...prev,
        name: session.user.name || '',
        email: session.user.email || '',
        // Try to get phone from user object if available
        phone: (session.user as any)?.phone || prev.phone,
      }))

      // Then load saved addresses
      fetch('/api/addresses')
        .then(res => res.json())
        .then(data => {
          if (Array.isArray(data)) {
            ...
            if (defaultAddress) {
              ...
              setFormData(prev => ({
                ...
                district: defaultAddress.city?.toLowerCase() || '',
              }))
            }
          }
        })
